...
def extract_topics(texts, num_topics=1, num_words=5):
...
def create_prompt(page_numbers, combined_text, topics):
    ...
    topics_text = "\n".join([f"Topic {i + 1}: {topic} (Pages {page_numbers})" for i, (_, topic) in enumerate(topics)])
    ...
    1. Topic: Provide a concise and insightful topic name (one line) associated with the page group number you extracted from. Ensure that the topic is insightful and relevant to the content of the pages. Make sure that each topic name must be unique .Ensure each and every topic generated is properly associated with its corresponding page group number. Also ensure the page group number is properly associated with its topic. The topic to be generated till the last page of the page group. 

    2. Subtopics: List 3-5 subtopics each that describe the main content of these page groups in a line without any extra points added to the subtopic. 
...
if pdf_file is not None:
    ...
    with st.container():
        ...
        for i, topic_data in enumerate(topics_data):
            ...
            subtopics_without_pages = [subtopic for subtopic in topic_data['subtopics']]
            with expander:
                st.write("Subtopics:")
                for subtopic in subtopics_without_pages:
                    st.write(subtopic)
